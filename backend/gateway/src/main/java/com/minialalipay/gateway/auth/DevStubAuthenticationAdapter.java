 ...
 *   <li>主体与角色来自受控配置（{@code stub-user}/{@code stub-roles}），不读取客户端身份头</li>
...
public final class DevStubAuthenticationAdapter implements GatewayAuthenticationPort {
    ...
    public DevStubAuthenticationAdapter(
            @Value("${gateway.authentication.stub-token:dev-admin-token}") String stubToken,
            @Value("${gateway.authentication.stub-user:dev-admin}") String principalId,
            @Value("${gateway.authentication.stub-roles:ADMIN}") String stubRoles) {
