...
import com.minialalipay.user.interfaces.filter.UserCenterRequestIdFilter;
...
/**
 * 将用户中心接口异常转换为统一中文响应，禁止向调用方暴露内部异常消息。
 */
@RestControllerAdvice
public class UserCenterExceptionHandler {

    private static final Logger LOGGER = LoggerFactory.getLogger(UserCenterExceptionHandler.class);

    private final CommonExceptionMapper exceptionMapper;
    private final RequestIdGenerator requestIdGenerator;

    public UserCenterExceptionHandler(
    ...
    private String resolveRequestId(HttpServletRequest request) {
        Object requestId = request.getAttribute(UserCenterRequestIdFilter.REQUEST_ATTRIBUTE);
        return requestId instanceof String value
                ? value
                : requestIdGenerator.resolve(request.getHeader(UserCenterRequestIdFilter.HEADER_NAME));
    }
}
