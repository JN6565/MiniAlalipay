...
import com.minialalipay.business.interfaces.filter.BusinessCenterRequestIdFilter;
...
/**
 * 将业务中心接口异常转换为统一中文响应，禁止向调用方暴露内部异常消息。
 */
@RestControllerAdvice
public class BusinessCenterExceptionHandler {

    private static final Logger LOGGER = LoggerFactory.getLogger(BusinessCenterExceptionHandler.class);

    private final CommonExceptionMapper exceptionMapper;
    private final RequestIdGenerator requestIdGenerator;

    public BusinessCenterExceptionHandler(
    ...
    private String resolveRequestId(HttpServletRequest request) {
        Object requestId = request.getAttribute(BusinessCenterRequestIdFilter.REQUEST_ATTRIBUTE);
        return requestId instanceof String value
                ? value
                : requestIdGenerator.resolve(request.getHeader(BusinessCenterRequestIdFilter.HEADER_NAME));
    }
}
