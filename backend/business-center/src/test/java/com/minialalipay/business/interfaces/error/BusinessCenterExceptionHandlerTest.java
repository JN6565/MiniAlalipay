package com.minialalipay.business.interfaces.error;
...
@Import(BusinessCenterFailingTestController.class)
class BusinessCenterExceptionHandlerTest {
    ...
    void mapsBusinessExceptionToStandardChineseResponse() throws Exception {
        mockMvc.perform(get("/test/business-error").header("X-Request-Id", "request-business-001"))
                .andExpect(status().isBadRequest())
                .andExpect(header().stringValues("X-Request-Id", "request-business-001"))
                .andExpect(content().contentTypeCompatibleWith(MediaType.APPLICATION_JSON))
                .andExpect(jsonPath("$.code").value("COMMON_INVALID_REQUEST"))
                .andExpect(jsonPath("$.message").value("请求参数不合法"))
                .andExpect(jsonPath("$.requestId").value("request-business-001"))
    ...
    void hidesUnexpectedExceptionDetails() throws Exception {
        mockMvc.perform(get("/test/internal-error").header("X-Request-Id", "request-business-500"))
                .andExpect(status().isInternalServerError())
                .andExpect(header().string("X-Request-Id", "request-business-500"))
    ...
    void keepsNotFoundHttpSemantics() throws Exception {
        mockMvc.perform(get("/test/not-found").header("X-Request-Id", "request-business-404"))
                .andExpect(status().isNotFound())
                .andExpect(header().string("X-Request-Id", "request-business-404"))
