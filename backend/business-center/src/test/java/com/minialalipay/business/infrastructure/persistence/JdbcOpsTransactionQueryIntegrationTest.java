...
class JdbcOpsTransactionQueryIntegrationTest {
    ...
    void 链路片段包含交易受理TCC全局与终态事件() {
        ...
        List<TraceSpan> spans = store.findTraceSpans("tx-1");
        assertThat(spans).hasSize(3);
        assertThat(spans.get(0).operation()).isEqualTo("统一交易受理");
        assertThat(spans.get(0).traceId()).isEqualTo("abcdef0123456789abcdef0123456789");
        assertThat(spans).anyMatch(s -> s.operation().equals("TCC 全局事务"));
        assertThat(spans).anyMatch(s -> s.operation().equals("终态事件发布"));
    }

    @Test
    void 不存在的交易返回空() {
        assertThat(store.findTransactionForOps("missing")).isEmpty();
        assertThat(store.findTraceSpans("missing")).isEmpty();
    }
