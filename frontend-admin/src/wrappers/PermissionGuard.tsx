 ...
 * 而跳转），再在布局渲染阶段（useLayoutEffect，先于 paint）检查布尔权限：
 * 未通过时用 history.replace 跳转到 redirectTo（默认 403 页）并返回 null 不渲染子页面。
 */
export default function PermissionGuard({ allowed, redirectTo = '/admin/403' }: PermissionGuardProps) {
  ...
  }, [allowed, identityLoaded, redirectTo]);
