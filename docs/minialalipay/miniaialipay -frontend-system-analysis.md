...
| 收款人选择 | 从搜索结果或常用联系人中选择；搜索结果与最近转账均展示脱敏真实姓名和脱敏手机号，未绑定身份时降级展示昵称 | 列表选择 | Y | — | — | 仅可选择列表中用户 | object | "请选择收款人" | 搜索结果/GET /contacts |
...
| 方法 | 路径 | 说明 |
| --- | --- | --- |
| GET | `/api/v1/users/search?keyword=` | 按手机号精确搜索用户，最多返回 10 条结果；返回脱敏真实姓名（仅用于收款确认展示）与脱敏手机号，明文姓名与完整手机号不下发 |
| GET | `/api/v1/contacts` | 查询成功转账历史生成的常用收款人，按次数/最近成功时间/置顶排序；返回脱敏展示名与脱敏手机号 |
