...
| V2.0 | 2026-08-11 | 项目组 | Mini 花呗改为用户显式开通；新增花呗商户收款码，未开通收款码的个人码和固定金额请求仅支持余额/银行卡 |
...
MVP 以“AI 发起转账并在故障下保持账实一致”为主故事，覆盖用户中心、业务中心、账户中心、AI Agent、MCP 工具服务、TCC/Saga 分布式事务、人工确认、链路追踪、实时数据看板和 T+1 离线报表。产品同时向普通用户提供动态扫码收款、长期个人收款码与 30 分钟固定金额收款请求，以及 Mini 花呗固定虚拟授信。主动转账只使用虚拟余额；动态扫码、个人码和固定请求付款均由用户明确选择虚拟余额或 Mini 花呗。所有环节只处理演示虚拟资金，不接入真实人民币、征信或放贷通道。
...
- Mini 花呗转账、自付或提现；信用支付只允许用于另一普通用户创建的动态扫码、个人码或固定金额收款订单。
- 多币种、跨境支付和复杂汇率结算。
- 生产级容灾、多地域部署和百万级并发。
...
| FR-UC-002 | 常用收款人与用户搜索 | P0 | 待开发 | 支持手机号精确查询 |
...
| FR-PC-003 | C2C 收款支付与一致性 | P0 | 待开发 | 用户选择余额或 Mini 花呗，统一 TCC 和复式账本 |
...
7. **入口不同、交易核心统一**：主动转账使用 `TRANSFER`；个人码和固定请求按用户选择创建 `TRANSFER` 或 `CREDIT_PAY`，手续费为 0。
...
- 用户名/密码注册、登录和退出，注册后自动开户且初始余额为 0；余额仅能通过已完成交易获得，真实资金流入渠道为他人转账与银行卡充值；受控模拟充值仅保留后端测试通道，C 端不再暴露入口。
- 6 位支付密码设置、校验与错误次数控制。
- 用户资料与模拟实名认证状态；C 端可在个人资料页调整昵称、预设头像或选择本地图片，图片仅作用于当前浏览器的展示偏好，不上传、不修改账户、身份或交易事实。
- 按手机号精确查询用户及管理常用收款人。
- 登录设备与最近活动记录。
...
- 个人码与固定请求 H5 由付款用户明确选择虚拟余额或 Mini 花呗，分别创建 `TRANSFER` 或 `CREDIT_PAY`，双方明细和回执同步更新。
...
| 长期个人码 | 付款方填写 | 展示个人码 | 扫码、填写金额/备注、选择资金来源并确认 | `TRANSFER`/`CREDIT_PAY` |
| 固定收款请求 | 收款方锁定 | 填写金额/备注并分享 | 打开链接或扫码后确认 | `TRANSFER` |

三种入口均禁止向本人同一账户付款，主动转账资金来源只允许 `BALANCE`，个人码和固定请求允许用户选择 `BALANCE` 或 `MINI_CREDIT`，手续费固定为 0；C2C 不提供群收款、AA 收款或部分收款进度。
...
用户使用唯一登录名和登录密码注册、登录。注册成功后自动开户，初始可用余额为 0。受控模拟充值通过“虚拟资金发行账户 → 用户余额账户”生成复式分录；充值必须限额、限流、幂等并记录审计日志，前端不得直接增加余额。模拟充值不符合真实资金流转，C 端页面与入口已下线，仅保留后端接口供测试环境演示资金注入；真实资金流入渠道为他人转账与银行卡充值。
...
系统支持按手机号精确查询收款人。搜索结果只用于本次选择；最近收款人和常用收款人仅基于成功转账历史生成，用户可置顶、隐藏或设置备注。

验收标准：

- 手机号必须精确匹配，返回对应 ACTIVE 状态的用户。
- 前端和日志默认隐藏完整手机号与账户号。
...
AI 支持“转账”“查余额”“查收支明细”“查交易状态”“查用户”“查 Mini 花呗额度”“查 Mini 花呗账单”“Mini 花呗还款”八类意图。转账意图至少抽取收款人、金额、备注三个槽位；收支明细支持时间范围、方向和交易状态；还款意图至少抽取还款金额，禁止模型默认选择“全部还清”。

当一条消息同时包含多个相互独立的只读查询（例如“查花呗、查余额、查交易”）时，Agent 必须为每个查询调用对应工具，查询结果全部返回后再统一回复；任何工具失败或返回字段缺失时只能明确说明未能查询，不得使用默认值或模型推测补齐事实。涉及转账、还款等资金操作时，资金流程保持既定串行顺序。
...
用户可以询问“查一下手机号 138****0001 的人”。AI 调用受控查询工具返回脱敏候选列表；用户明确选择后，候选用户才可进入转账草稿。

#### FR-AI-007 用户偏好记忆（P1）

经用户明确同意后，系统可记忆常用收款人、常用备注等低敏偏好。偏好只用于候选排序，不得自动确定收款人、金额或跳过确认；用户可查看并删除全部偏好。

#### FR-AI-008 SSE 流式响应（P1）

AI 服务通过 `POST /api/v1/agent/messages/stream`（`text/event-stream`）逐阶段推送处理进度：
意图识别 → 工具调用 → 工具结果 → 自然语言片段 → 确认卡片/澄清引导 → 完成。

验收标准：
- 流式端点与同步端点共享同一业务逻辑和策略校验。
- 前端使用 fetch + ReadableStream 消费 POST SSE 流，兼容主流移动浏览器。
- SSE 事件不传输支付密码、确认令牌或完整账号。
- 断线或超时时前端自动显示错误提示。

#### FR-AI-009 确认卡片交互（P1）

AI 在完成转账草稿校验和确认卡片生成后，向前端推送结构化确认卡片。
用户点击确认后输入 6 位支付密码完成提交。AI Agent 不接触支付密码。

验收标准：
- 确认卡片展示收款人昵称、脱敏手机号、金额和备注。
- 用户在确认前可取消操作。
- 支付密码通过可信 UI 验证，不在 SSE 事件中传输。
...
个人码和固定请求付款由付款用户明确选择资金来源：`BALANCE` 创建 `business_type=TRANSFER`，`MINI_CREDIT` 创建 `business_type=CREDIT_PAY`。手续费固定为 0，客户端指定收款账户或未明确选择资金来源均被拒绝。
...
| T+1 报表页 | 日期筛选、离线指标、生成状态、口径说明、交易总览、趋势、脱敏对账、质量和告警详情 | P0 |
...
| C2C 收款 H5 | 脱敏收款人、金额、备注、手续费 0、余额/花呗资金来源、可用余额/额度、支付密码、订单状态 | 个人码模式允许付款人填写并一次锁定金额；固定请求金额/备注只读；用户必须明确选择余额或 Mini 花呗；SSE 失败降级轮询 |
...
| C2C 个人收款 | 个人码扫码/确认/成功率、固定请求创建/支付/过期率、成功金额、并发冲突和同步延迟 | 按 `PERSONAL_QR_ORDER`/`COLLECTION_REQUEST_ORDER`、`source_type`、`fundingSource` 和结果分维度，资金交易归入 `TRANSFER` 或 `CREDIT_PAY` |
| 链路检索 | Trace ID、Transaction ID、阶段、耗时、返回码 | 输入精确 ID 查询；敏感字段统一脱敏 |
| 离线报表 | 数据日期、登录成功率、注册转化、支付成功率、余额不足占比 | 默认展示最近 7 个数据日；标明生成状态和口径版本。系统管理员可生成“昨日零点至当前时刻”的临时预览，预览不覆盖正式 T+1 报表且须展示质量门禁状态与数据范围；失败时可携带本次只读检查结果进入数据质量页查看，必须明确该结果未写入正式质量记录；已发布日报详情还需展示交易总览、按小时趋势、脱敏对账差异、质量维度及告警汇总。交易金额只累计按交易号去重后的最终 `SUCCESS` 交易；可信运行看板与 T+1 使用同一口径，趋势按上海业务日整点每小时聚合一次。 |
...
| `GET /api/v1/users/search` | 按手机号查询收款人 | 仅返回脱敏数据 |
...
| `POST /api/v1/p2p-collections/orders/{id}/pay` | 提交 C2C `TRANSFER` 或 `CREDIT_PAY` | `Idempotency-Key`、令牌消费、订单/请求 CAS、源订单唯一、TCC、Outbox |
...
- C2C 令牌交换、订单修改、确认和付款分别限流。服务端忽略客户端传入的收款账户，固定请求拒绝金额/备注覆盖，确认令牌绑定用户明确选择的 `BALANCE` 或 `MINI_CREDIT`。
...
| C2C 收款成功金额 | `SUCCESS TRANSFER/CREDIT_PAY` 按 `PERSONAL_QR_ORDER`/`COLLECTION_REQUEST_ORDER` 来源汇总 | 请求、订单、交易与账本事件 | 余额扣减或信用应收增加 = 收款增加 = 借贷金额 |
...
| AT-65 | C2C 收款支付成功 | 余额支付减少付款方余额；花呗支付占用额度并增加信用应收；收款方均等额增加，手续费为 0，借贷平衡 |
...
| C2C 跨资金来源重复支付 | 同一订单余额和花呗各成功一次 | C2C 订单来源唯一键、订单 CAS 和确认令牌绑定资金来源 |
...
| CollectionOrder | 个人码或固定请求的一次独立付款尝试，也是重复判断和 `TRANSFER`/`CREDIT_PAY` 的来源订单 |
...
## 需求补充：Mini 花呗开通与商户收款码

本补充覆盖 2026-08-11 确认的 Mini 花呗链路。系统可以在开户注册时预创建固定 5,000.00 元演示额度账户，但 `opened=false` 时视为未开通，用户必须在 C 端点击“确认开通”后才能使用 Mini 花呗付款。未开通、暂停、逾期或额度不足时，任何 `MINI_CREDIT` 支付确认都必须拒绝。

花呗商户收款码是普通用户个人收款能力的开关，不引入独立商户角色。收款方开通后，其长期个人收款码和固定金额收款请求才允许付款方选择 Mini 花呗；未开通时，付款页展示余额和银行卡，Mini 花呗置灰并提示“该收款码暂不支持花呗”。开通动作幂等，换码后继承已开通状态。

Mini 花呗付款成功后必须形成 `CREDIT_PAY`：付款方 Mini 花呗账单和首页账单显示支出，收款方余额明细显示收入入账；额度不计入余额，不可提现，不可普通转账。
...
5. 主动转账只使用 `TRANSFER`；长期个人码和固定请求付款按用户选择使用 `TRANSFER` 或 `CREDIT_PAY`，来源对象、订单和状态机独立建模，以同时支持个人码多笔合法付款与固定请求的独立付款订单。
