...
| 离线报表 | 数据日期、登录成功率、注册转化、支付成功率、余额不足占比 | 默认展示最近 7 个数据日；标明生成状态和口径版本。系统管理员可生成“昨日零点至当前时刻”的临时预览，预览不覆盖正式 T+1 报表且须展示质量门禁状态与数据范围；失败时可携带本次只读检查结果进入数据质量页查看，必须明确该结果未写入正式质量记录 |
