...
所有金额指标使用“分”聚合，前端显示时转换为元。四类交易均进入账本、Trace 和对账指标，并通过 `business_type` 分维度展示。`CREDIT_PAY` 的收款用户入账属于虚拟资金增加且同时形成等额信用应收，`CREDIT_REPAY` 同时减少用户虚拟余额和信用应收；统计必须避免把额度本身当作资金余额或支付金额。
