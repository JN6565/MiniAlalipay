...
| V1.19 | 2026-08-10 | 项目组 | 新增银行卡虚拟余额与充值提现功能：银行卡聚合根扩展 `balance_fen` 字段，支持充值（银行卡给账户充钱，银行卡虚拟余额减少 + 账户余额同步增加）、提现（账户给银行卡充钱，反向）、银行卡余额转账和扫码支付；充值/提现走 Seata TCC 全局事务，单笔 0.01–50000.00 元，日累计充值/提现各 50000.00 元；转账和扫码支付接口增加 `fundingSource`/`cardId` 参数支持银行卡扣款 |
...
- 手机号、真实姓名、登录密码、支付密码注册，手机号或系统账户号登录，自动开户（初始余额为 0）和受控模拟充值（C 端入口已下线，仅保留后端测试通道）。
...
| UC-01A | 模拟充值（仅后端测试通道，C 端入口已下线） | 登录用户 | 账户正常、未超过限额 | 充值交易、余额、账本和审计记录一致 |
...
- 银行卡绑定管理：注册（选银行 + 三要素 → 须先绑定身份且三要素与已绑定身份交叉比对一致，通过后自动生成卡号）、绑卡（基于注册记录、跨服务三要素校验）、卡列表、卡详情、设默认与解绑；只保存 BIN、尾号与掩码，注册记录状态 `REGISTERED ⇄ BOUND`（绑卡置 BOUND，解绑时释放回 REGISTERED 支持重绑）、绑卡记录状态 `ACTIVE → UNBOUND`（绑定记录终态，重绑生成新记录），同一用户至多一张默认卡，解绑默认卡后递补最早活动卡；用户注册后需先完成身份绑定（姓名 + 身份证号）才能进行银行卡注册和绑定。身份证校验全系统统一口径：18 位格式 + 出生日期真实存在且介于 1900-01-01 至今，不做 MOD 11-2 校验码；未绑定身份注册卡返回 `IDENTITY_NOT_BOUND`，三要素不一致返回 `IDENTITY_MISMATCH`（统一提示不区分字段）。
- 银行卡虚拟余额与充值提现：每张银行卡拥有独立的虚拟余额（`balance_fen`），与账户余额分离。充值即银行卡给账户充钱，银行卡虚拟余额减少、账户余额同步增加；提现即账户给银行卡充钱，账户余额减少、银行卡虚拟余额同步增加。充值/提现均通过 Seata TCC 全局事务保证一致性：Try 阶段检查限额和余额充足性并冻结，Confirm 阶段实际扣减/增加，Cancel 阶段回滚。严格限额：单笔 0.01–50000.00 元，日累计充值/提现各 50000.00 元。银行卡虚拟余额也可用于转账和扫码支付的资金来源（`fundingSource=BANK_CARD`），此时 TCC 分支注册银行卡余额扣减而非付款方账户冻结。
...
| 模拟充值（C 端入口已下线，仅后端测试通道） | 后端接口 | 校验登录态、限额、幂等与模拟渠道结果 | 发行账户向用户余额账户入账并写平衡分录 | `recharge_order`、`fund_transaction`、`voucher`、`ledger_entry` | 充值状态、余额、明细和回执 |
| 银行卡充值/提现 | C 端 H5 | 校验限额、幂等，发起 Seata 全局事务 | 充值：银行卡余额减少 + 账户余额增加；提现：账户余额减少 + 银行卡余额增加。TCC Try 校验并冻结，Confirm 实际变动 | `bank_card_recharge_order`/`bank_card_withdraw_order`、`bank_card_daily_usage`、`fund_transaction` | 充值/提现结果、银行卡余额变化 |
| 银行卡余额转账/扫码 | C 端 H5 | 转账草稿/扫码订单携带 `fundingSource=BANK_CARD` + `cardId` | TCC 分支注册银行卡余额扣减（替代付款方账户冻结），收款方正常入账 | `fund_transaction`（`funding_source=BANK_CARD`） | 交易状态、回执、银行卡余额变化 |
...
| POST | `/api/v1/recharges` | 登录用户 | 创建模拟充值订单（C 端入口已下线，仅保留后端测试通道；真实充值使用 `/api/v1/bank-cards/{cardId}/recharge`） | `X-Request-Id`、`Idempotency-Key`；单笔不超过 `5000000` 分，单用户单日累计不超过 `25000000` 分且最多 `5` 次；渠道成功后由充值 TCC Confirm 增加余额并完成 RECHARGE 复式记账 |
