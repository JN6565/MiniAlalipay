...
- 银行卡绑定管理：注册（选银行 + 三要素 → 须先绑定身份且三要素与已绑定身份交叉比对一致，通过后自动生成卡号）、绑卡（基于注册记录、跨服务三要素校验）、卡列表、卡详情、设默认与解绑；只保存 BIN、尾号与掩码，注册记录状态 `REGISTERED ⇄ BOUND`（绑卡置 BOUND，解绑时释放回 REGISTERED 支持重绑）、绑卡记录状态 `ACTIVE → UNBOUND`（绑定记录终态，重绑生成新记录），同一用户至多一张默认卡，解绑默认卡后递补最早活动卡；本期不作为资金来源，不参与资金记账。用户注册后需先完成身份绑定（姓名 + 身份证号）才能进行银行卡注册和绑定。身份证校验全系统统一口径：18 位格式 + 出生日期真实存在且介于 1900-01-01 至今，不做 MOD 11-2 校验码；未绑定身份注册卡返回 `IDENTITY_NOT_BOUND`，三要素不一致返回 `IDENTITY_MISMATCH`（统一提示不区分字段）。
...
| Bank Card | BankCard/RegisteredCard | 银行卡注册与绑定管理、BIN 识别、掩码存储、默认卡互斥、解绑释放注册记录支持重绑、跨服务三要素校验 |
...
sequenceDiagram
    ...
    alt H5 合并提交（单请求）
        ...
        B->>UC: 验密并签发证明（服务间调用）
        UC-->>B: 一次性支付证明
        B->>B: 签发并消费一次性确认令牌，受理 TRANSFER
    else 三步流程（AI Talk 等场景）
        W->>UC: 校验支付密码
        UC-->>W: 短期密码校验凭证
        W->>B: 生成一次性确认令牌
        W->>B: 提交 TRANSFER + 幂等键
...
个人码长期复用，每个付款会话对应独立订单，因此两名付款人可并发成功。付款账户由登录会话确定，收款账户由个人码确定；自付、客户端收款账户和非正金额均拒绝，资金来源由付款用户明确选择。个人码同时分配 8 位数字手动输入短码，长期有效，换发后旧码失效；付款人可在扫一扫页手动输入短码等价进入付款流程（见 9.4.5）。
...
一码多收下扫码阶段不占用请求：每次扫码都新建独立订单，会话旧订单一律解绑不复用；支付受理只校验请求未过期且未取消，多笔并发受理由订单 CAS 与请求版本 CAS 保护。单笔余额不足或失败只影响该笔订单，不阻断其他付款人；收款方通过订单列表接口实时看到每笔脱敏付款人与状态。固定请求同样分配手动输入短码，有效期与请求一致（30 分钟），请求取消或过期后短码同步失效（见 9.4.5）。

#### 9.4.5 手动输入收款短码

付款人浏览器无法使用摄像头时，三类码（长期个人码、固定金额请求、动态扫码收款订单）在生成时同步分配 8 位纯数字短码，收款页展示短码并提示「对方也可手动输入此码完成支付」；登录付款人经 `POST /api/v1/p2p-collections/short-code-exchanges` 兑换，结果与扫描对应二维码等价：

- 生成：SecureRandom 生成 8 位纯数字；事务内碰撞重试上限 5 次；生成前先清理同表已过期/失效记录占用的短码。
- 有效期随宿主实体：个人码长期有效（换发新码时旧码失效）、固定请求 30 分钟、动态订单 5 分钟；宿主实体进入不可支付状态时短码同步清理。
- 唯一性：三张表各自的 `short_code` 唯一索引（允许 NULL，仅非空唯一）。
- 防猜测：短码只是订单指针而非凭据，明文存储，付款仍需登录、验密与风控全套保护；码不存在、已过期、已停用或已消费一律统一为 `SHORT_CODE_INVALID`（404），不暴露码状态差异；Redis 按登录身份计数，10 分钟内失败 5 次锁定 30 分钟返回 `SHORT_CODE_RATE_LIMITED`（429），Redis 故障时 fail-open。
- 兑换响应只返回码类型与订单 ID，不返回原始令牌（令牌只存摘要）；付款页按订单 ID 直接进入支付流程，无需 bootstrap 与令牌交换。
...
注册卡记录状态：`REGISTERED → BOUND`（绑卡成功），解绑时释放回 `REGISTERED`，支持同一张卡重新绑卡。绑卡记录状态：`ACTIVE → UNBOUND`（绑定记录终态），同一实体卡解绑后如需再使用必须凭注册时的完整卡号与三要素重新走绑卡流程生成新记录。同一用户至多一张 ACTIVE 默认卡，解绑默认卡后自动递补最早绑定的活动卡。本期银行卡仅用于展示管理，不作为资金来源参与资金记账。绑卡时需跨服务调用 user-center 校验三要素与用户存储身份完全匹配。
...
接口业务分组用于确定后端所有者，调用端分组用于确定客户端、角色和数据范围，两者不能混为一谈。同一个 `business-center` 可以同时提供 C 端交易接口和 B 端运营接口，但必须使用不同权限模型。
...
| 内部服务 | 开户、用户解析、TCC 分支、账本核验、终态查询 | 服务身份和业务对象双重授权 | 暴露给浏览器、跨 Schema 直写或共享聚合根和持久化对象 |
...
| GET | `/api/v1/users/me` | 登录用户本人 | 查询账户名、昵称、脱敏手机号和脱敏真实姓名 | 只读；会话主体派生用户，完整手机号与真实姓名不出服务边界 |
...
| GET | `/api/v1/accounts/me/entries` | 登录用户 | 查询本人账本明细，含脱敏交易对方 `counterpartyName` 和交易后余额 `balanceAfterFen`（可空，存量与系统账户分录无值） | `cursor` + `limit<=100` |
...
| POST | `/api/v1/transfers/submit-with-password` | 付款人本人（H5 主动转账） | 验密即支付：服务端内部完成证明签发、确认令牌签发与消费、受理 | 强制幂等键；支付密码仅走请求体，不进日志、URL 或存储 |
...
两个接口只允许业务中心或账户中心通过服务间网络和服务身份调用，不注册到前端网关。主体、用途、版本或状态不匹配统一按
`PAYMENT_PROOF_INVALID` 处理，避免向调用方泄露证明是否属于其他用户。

#### Mini 花呗阶段五公开接口与还款一致性

账户中心通过网关公开额度摘要、消费明细、账单列表、账单详情、还款草稿、还款提交和还款状态七个 C 端接口，并公开月度出账、到期检查两个 B 端运维接口。路径、请求字段、金额单位和错误响应以 OpenAPI 为唯一事实来源。

还款写操作遵守以下边界：

1. 创建草稿时按“逾期账单、已出账账单、未出账消费”计算分配顺序，并将分配快照和 SHA-256 摘要固化；同一用户、同一幂等键重复请求只能返回同一草稿。
2. 提交还款必须携带用途为 `CREDIT_REPAY` 的一次性支付证明。账户中心只向用户中心传递原始证明，不写日志、不持久化；用户中心校验成功后原子消费。
3. 还款 Confirm 必须使用草稿固化的分配，不得重新计算；父分配金额必须等于逐笔消费明细合计，还款总分配必须等于余额扣减、应收减少和额度恢复金额。
4. 逾期账单部分还款后仍保持 `OVERDUE`，只有全部清偿后转为 `PAID`；账户不存在其他逾期未还账单时才恢复 `ACTIVE`。
5. 上月消费在本月 1 日出账，并在本月 10 日到期，不得把到期时间误设为账期月份而造成新账单立即逾期。
...
银行卡出资（确认阶段绑定 `fundingSource=BANK_CARD` 与 `cardId`）时，受理交易的付款账户为空，资金从银行卡虚拟余额扣减；响应 `businessType` 仍为 `TRANSFER`。
...
对于同一条消息中的两个及以上独立只读查询，Planner 生成 `ToolCalls` 批次并发执行；批次内每个工具仍经过相同的登录、对象权限、注入检测和审计。批次结果按用户表达顺序逐项展示，回复直接使用结构化结果解释，不再让模型改写事实。任何资金草稿、校验或提交工具不得进入并行批次，保持转账链路的顺序和幂等边界。
...
- C 端 AI 转账确认卡以整数分校验 `1..min(实时可用余额, 5000000)`；卡片加载和点击确认时均经网关回源 `/api/v1/accounts/me`，余额未知或查询失败时禁用确认。前端校验只用于即时反馈，服务端草稿校验和执行前余额复检仍是最终安全边界。
...
- 工具结果缺少事实字段时返回“数据不完整/暂时无法查询”，禁止以 `0`、固定额度或历史值代替缺失字段，避免 AI 臆测。
...
本地演示支持网关 dev Stub（仅 `GATEWAY_AUTH_STUB_ENABLED=true` 时激活，令牌默认 `dev-admin-token`，角色默认 `ADMIN`）；默认关闭（false），登录发放的会话令牌一律回源用户中心 `introspect` 校验——若误开 Stub，真实会话会被固定令牌比对拒为 401，表现为登录后立即弹回登录页。生产构建不注入任何演示令牌，未登录一律引导到登录页。用户列表/冻结/解冻接口仍待用户中心合入后接通，在此之前用户管理页保持诚实占位、不发起请求。
