...
| V1.16 | 2026-08-08 | 项目组 | 银行卡注册接入三要素交叉校验：须先绑定身份且姓名、身份证哈希、手机号与已绑定身份一致；统一身份证校验口径（格式+出生日期，不做 MOD 11-2 校验码） |
| V1.17 | 2026-08-08 | 项目组 | 完善银行卡绑定全流程：解绑时同步释放注册记录（BOUND → REGISTERED），支持解绑后凭完整卡号与三要素重新绑定；重绑复用现有绑卡接口并生成新绑定记录 |
