...
| V1.4 | 2026-07-28 | 项目组 | 纠正扫码支付资金语义：改为通过 TCC 扣减付款方虚拟余额、增加商户虚拟余额并写入复式账本 |
...
MVP 以“AI 发起转账并在故障下保持账实一致”为主故事，覆盖用户中心、业务中心、账户中心、AI Agent、MCP 工具服务、TCC/Saga 分布式事务、人工确认、链路追踪、实时数据看板和 T+1 离线报表。产品同时提供商户动态收款码、普通用户长期个人收款码与 30 分钟固定金额收款请求，以及 Mini 花呗固定虚拟授信。主动转账、个人码付款和固定请求付款统一执行 `TRANSFER`；商户扫码可选择虚拟余额或 Mini 花呗。所有环节只处理演示虚拟资金，不接入真实人民币、征信或放贷通道。
...
| G-07 | 建立跨端扫码支付闭环 | 手机扫码后完成虚拟账户支付，付款方虚拟余额减少、商户虚拟余额增加，Web 实时显示结果且账本一致 |
| G-08 | 建立最小虚拟信用消费闭环 | 固定授信用户可使用 Mini 花呗向商户支付、查询账单并使用虚拟余额还款，额度和账本始终一致 |
...
| FR-AC-001 | 自动开户与虚拟金初始化 | P0 | 待开发 | 初始 10000 元 |
...
| FR-SP-001 | Web 创建扫码支付订单与动态收款码 | P0 | 待开发 | 商户账户、金额、5 分钟令牌 |
| FR-SP-002 | 手机扫码进入 H5 支付页 | P0 | 待开发 | 系统相机打开链接 |
| FR-SP-003 | H5 支付确认与资金处理 | P0 | 待开发 | 支付密码、风控、TCC、复式账本 |
| FR-SP-004 | Web 支付结果同步与监控 | P0 | 待开发 | SSE 推送、轮询兜底 |
| FR-CR-001 | Mini 花呗开通与额度 | P0 | 待开发 | 演示用户固定 5,000 元虚拟额度 |
| FR-CR-002 | Mini 花呗扫码支付 | P0 | 待开发 | 仅商户消费，不可转账或提现 |
...
| 角色 | 核心诉求 | 权限边界 |
|---|---|---|
| 普通用户 | 快速、安全地转账、创建个人收款入口并查询账户 | 仅操作本人账户和收款对象；付款必须本人确认，收款不代表好友关系 |
| 模拟商户 | 生成动态收款码并查看收款结果 | 只能为本人商户虚拟账户创建订单，不可指定任意收款账户 |
...
- 用户名/密码注册、登录和退出，注册后自动开户并初始化 10000 元虚拟金。
- 6 位支付密码设置、校验与错误次数控制。
- 用户资料与模拟实名认证状态。
...
- 信用额度只能用于扫码商户消费，不能提现、转账或作为虚拟余额展示。
...
1. 商户登录 Web 收银台，为本人商户虚拟账户填写金额和商品说明，后端创建 `QR_PAY` 扫码支付订单。
...
5. H5 使用令牌查询脱敏订单信息，展示商户、金额、商品说明、剩余有效期和“本项目仅使用虚拟资金，不会扣除真实人民币”提示。
...
8. 业务中心使用幂等键创建统一交易单：余额支付为 `QR_PAY`，Mini 花呗支付为 `CREDIT_PAY`。二者通过 TCC 增加商户虚拟余额并写入借贷平衡分录；前者扣减付款方虚拟余额，后者占用授信额度并增加用户信用应收。
9. 交易成功后，Web 通过 SSE 收到 `SUCCESS` 结果并展示收款回执；失败时按统一交易规则重试、补偿或转人工，SSE 不可用时每 2 秒轮询。

### 8.7 Mini 花呗支付与还款流程

1. 系统为演示用户开通固定 5,000.00 元虚拟额度，初始已用和冻结额度均为 0。
2. 用户在扫码 H5 选择 Mini 花呗并确认支付，系统通过 TCC 冻结可用额度、预占商户入账和账本凭证。
3. Confirm 后已用额度增加、商户虚拟余额增加，账本记录“借：用户信用应收；贷：商户虚拟余额负债”，消费进入当前未出账明细。
...
用户使用唯一登录名和登录密码注册、登录。注册成功后由用户中心调用账户中心自动开户，并通过账本初始化 10000.00 元虚拟金。系统签发短期访问令牌并记录设备、时间和 IP 摘要，同时保留演示账号快捷登录入口。
...
系统支持按登录名精确查询、按昵称模糊查询和按手机号尾号辅助查询，并允许用户将查询结果保存为常用收款人。
...
- 常用收款人是用户单向保存的快捷记录，不表示好友申请、对方同意或双向好友关系。
...
同一会话内保留最近 10 轮对话和当前转账草稿。跨会话只保留经用户确认的偏好，不保留支付密码、确认令牌和完整敏感信息。
...
#### FR-AC-001 自动开户与虚拟金初始化（P0）

用户注册成功时，系统自动创建人民币虚拟账户并通过复式账本赠送 10000.00 元虚拟金。初始化动作只允许执行一次，仅在演示环境开放，并记录审计日志。
...
| 业务 | 借方 | 贷方 |
|---|---|---|
| `TRANSFER` / 余额 `QR_PAY` | 付款方虚拟余额负债 | 收款方/商户虚拟余额负债 |
| `CREDIT_PAY` | 用户 Mini 花呗应收资产 | 商户虚拟余额负债 |
| `CREDIT_REPAY` | 用户虚拟余额负债 | 用户 Mini 花呗应收资产 |
| 虚拟金初始化 | 虚拟金发行/权益账户 | 用户虚拟余额负债 |
...
前端展示近 7 天/30 天收支趋势、收入支出汇总和交易对象分布。统计数据来自账本或分析事件，不以聊天文本为数据源；无数据时展示明确空状态。
...
| R-05 新交易对手 | 首次向该收款人/商户支付且金额大于等于 1,000 元 | 强风险提示；仍需支付密码 |
| R-06 重复特征 | 30 秒内向同一收款人/商户以相同金额重复发起 | 强提示并要求重新确认 |
...
1. Try：校验交易状态；按业务冻结虚拟余额或信用额度；预占商户入账、信用应收/还款分配和账本凭证。
2. Confirm：按业务扣减虚拟余额、占用/恢复额度、更新信用应收和账单、增加商户可用余额，写入双式账本后进入终态发布。
3. Cancel：释放余额/额度冻结，取消商户入账、信用应收/还款分配和账本预占，交易置为已撤销。
...
商户登录 Web 收银台后，为本人商户虚拟账户填写金额和商品说明并创建 `QR_PAY` 订单。后端生成订单号和 5 分钟有效的一次性二维码令牌，Web 将 H5 地址转换为动态收款码。

验收标准：

- 商户名称来自登录商户资料，不允许客户端自由指定收款账户；商品说明不超过 50 个字符，金额范围为 0.01–50,000.00 元。
...
- 页面展示商户名称、支付金额、商品说明、订单号、剩余有效期和“仅使用系统虚拟资金，不会扣除真实人民币”声明。
...
- 手机无法访问地址时，Web 收银台提示检查局域网地址或演示 HTTPS 域名，而不是持续等待扫码。

#### FR-SP-003 H5 支付确认与资金处理（P0）

用户选择资金来源、核对订单并输入支付密码后，服务端执行风控校验并签发绑定付款人、商户、金额、资金来源和订单哈希的一次性确认令牌。选择虚拟余额时创建 `QR_PAY`；选择 Mini 花呗时按 FR-CR-002 创建 `CREDIT_PAY`。两种资金来源必须复用同一订单受理、TCC、账本、恢复与对账核心。
...
- Try 阶段冻结付款方虚拟余额，预占商户入账和账本凭证；Confirm 阶段扣减冻结额、增加商户可用余额并写入借贷平衡分录；Cancel 阶段释放冻结和预占。
- 重复确认或并发使用不同资金来源 100 次，同一扫码订单只能产生一笔 `QR_PAY` 或 `CREDIT_PAY` 交易、一组商户入账和一组有效账本分录。
- 余额支付成功后，付款方虚拟余额减少、商户余额等额增加；信用支付成功后，付款方虚拟余额不变、已用额度增加、商户余额等额增加、平台信用应收等额增加。
- H5 回执展示“虚拟资金支付成功”、交易号、商户、金额和时间，不得表述为真实人民币扣款。
- TCC 无法自动收敛时，按统一 Saga 补偿、人工确认和证账实对账流程处理。

#### FR-SP-004 Web 支付结果同步与监控（P0）

Web 收银台通过 SSE 订阅扫码支付订单状态，依次展示“待扫码、已扫码、待确认、处理中、支付成功、已取消、已过期或人工确认中”。SSE 连接失败时降级为每 2 秒轮询，终态后立即停止。
...
- `QR_PAY` 与 `CREDIT_PAY` 均计入扫码支付，但必须按资金来源分维度展示，且商户入账金额不得重复统计。
...
- 商户收款账户不因开通 Mini 花呗而获得授信额度。

#### FR-CR-002 Mini 花呗扫码支付（P0）

用户扫描商户二维码后可选择 Mini 花呗作为资金来源。信用支付沿用同一 `QrPayOrder`，创建业务类型为 `CREDIT_PAY` 的交易，不得创建第二个扫码订单或绕过原订单金额。

验收标准：

- 仅 `ACTIVE` 且无逾期账单的额度账户可支付；可用额度不足时在确认前拒绝，虚拟余额、额度和商户余额均不变化。
- 信用支付必须输入支付密码并经过风控、确认令牌、订单 CAS、来源订单唯一约束和幂等受理。
- Try 阶段冻结信用额度、预占商户入账和账本凭证；Confirm 阶段冻结额度转为已用额度、商户虚拟余额增加；Cancel 释放冻结额度并取消预占。
- 信用支付复式分录为：借记“用户 Mini 花呗应收资产”，贷记“商户虚拟余额负债”。借贷金额必须相等，成功前必须过账。
- 信用支付成功不扣减用户虚拟余额；页面和回执必须分别展示“虚拟余额变化 0”“Mini 花呗已用额度增加”和“商户入账增加”。
...
| `SUCCESS` | 付款方扣款、商户入账和账本分录全部完成 | 终态 |
| `REJECTED` | 余额、账户、硬风控或人工确认拒绝，未发生资金变化 | 终态 |
| `CANCELLED` | 用户取消或补偿完成，资金已恢复 | 终态 |
| `EXPIRED` | 二维码令牌超过 5 分钟 | 终态 |

扫码订单在 `PROCESSING` 时关联一笔业务类型为 `QR_PAY` 或 `CREDIT_PAY` 的统一交易单，资金状态以交易核心为唯一事实来源。`RISK_REVIEW` 与 `MANUAL_REVIEW` 不得混用：前者无资金变化，后者可能存在余额或额度冻结。订单投影不得早于交易核心进入 `SUCCESS`；成功订单必须存在 TCC 分支、相应余额扣减或额度占用、商户入账和借贷平衡的 `LedgerEntry`。
...
| 页面 | 核心内容 | 优先级 |
|---|---|---|
| 注册页 | 登录名、登录密码、6 位支付密码、注册协议 | P0 |
| 登录页 | 登录名、登录密码、演示账号快捷入口 | P0 |
| 首页 | 总资产、Mini 花呗摘要、快捷转账、AI Talk 入口、最近交易 | P0 |
| Web 商户收银台 | 商户虚拟账户、金额、商品说明、动态收款码、跨端状态 | P0 |
| H5 扫码支付页 | 商户订单、资金来源、倒计时、风险提示、支付密码、确认与取消 | P0 |
...
- 信用支付成功：分别展示虚拟余额变化 0、已用额度增加、商户入账和信用应收分录，避免误认为已从余额扣款。
...
|---|---|---|
| 注册页 | 登录名、登录密码、确认密码、支付密码、确认支付密码 | 实时校验格式；提交后显示开户与虚拟金初始化结果；任一步失败不得进入首页 |
...
| Web 商户收银台 | 商户虚拟账户、金额、商品说明、二维码、剩余有效期、手机端和资金状态 | 创建后锁定订单字段；支持取消未支付订单和重新创建；通过 SSE 更新状态，失败时降级轮询 |
| H5 扫码支付 | 商户、金额、商品说明、订单号、余额/花呗资金来源、可用余额/额度、倒计时、风险提示、支付密码 | 切换资金来源会废弃旧确认；花呗逾期或额度不足时禁用并说明；成功后按资金来源展示真实余额/额度变化 |
...
| 扫码支付 | 订单创建数、扫码率、支付成功率、成功金额、平均确认时长、资金来源 | 按 `QR_PAY`/`CREDIT_PAY` 和 `funding_source` 展示，商户入账不重复统计 |
...
| QrPayOrder | qr_order_id、merchant_user_id、merchant_account_id、payer_user_id、transaction_id、funding_source、amount_fen、subject、status、version、scanned_at、confirmed_at、expires_at | 扫码支付订单与余额/信用交易二选一关联；版本号用于支付/取消/过期竞态控制 |
...
| CreditPurchase | purchase_id、credit_transaction_id、credit_account_id、qr_order_id、merchant_account_id、amount_fen、billing_status、occurred_at | 信用消费明细；credit_transaction_id 唯一 |
...
| `POST /api/v1/qr-pay/orders` | 商户创建扫码订单和二维码地址 | 商户权限、收款账户绑定本人、5 分钟有效 |
...
| `GET /api/v1/qr-pay/orders/{id}` | Web/H5 查询当前状态 | 商户或付款人权限隔离，资金状态回源交易核心 |
...
- 运营人员只能访问脱敏工单与必要链路信息。
- 管理员配置操作与资金账本权限分离。
- 普通用户只能查询本人的额度、消费、账单和还款；商户只能查看关联信用支付的收款结果，不能查看用户额度或其他账单。
...
- 二维码令牌使用密码学安全随机数，服务端存摘要并设置 5 分钟有效期；扫码地址不携带商户、金额、用户等可篡改业务字段。
...
全链路数据监控用于回答五个问题：系统是否正常、资金是否一致、信用额度/应收/账单是否一致、AI 是否按规则执行、业务数据是否可信。监控范围覆盖 API 网关、用户中心、业务中心、账户中心、Mini 花呗模块、商户扫码与 C2C 个人收款模块、TCC/Saga 协调器、MCP Server、Agent 引擎、实时链路和离线任务。

监控分为四层：

1. **系统层**：CPU、内存、线程池、数据库连接池、缓存、消息积压和服务可用性。
2. **交易层**：交易量、金额、成功率、在途时长、TCC 分支、Saga 补偿、账本平衡和对账差异，按四类业务交易分维度。
...
| `businessType` | 资金/信用事件必填，只允许 `TRANSFER`、`QR_PAY`、`CREDIT_PAY`、`CREDIT_REPAY`、`VIRTUAL_FUND_INIT`；用于实时、离线和对账分维度统计 |
| `fundingSource` | 扫码订单事件必填，取 `BALANCE` 或 `MINI_CREDIT`；非扫码事件允许为空 |
| `sourceType` | 来源型交易和 C2C 资金事件必填；主动转账、个人码订单、固定请求订单、商户扫码订单和信用还款分别使用 `TRANSFER_DRAFT`、`PERSONAL_QR_ORDER`、`COLLECTION_REQUEST_ORDER`、`QR_PAY_ORDER`、`CREDIT_REPAYMENT`；非资金生命周期事件沿用其聚合对应来源类型 |
...
所有金额指标使用“分”聚合，前端显示时转换为元。四类交易均进入账本、Trace 和对账指标，并通过 `business_type` 分维度展示。`CREDIT_PAY` 的商户入账属于虚拟资金增加且同时形成等额信用应收，`CREDIT_REPAY` 同时减少用户虚拟余额和信用应收；统计必须避免把额度本身当作资金余额或支付金额。
...
| 各业务交易金额 | 各 `business_type` 的 `SUCCESS` 金额分别汇总，不跨类型直接相加 | 交易与账本事件 | 转账、扫码商户入账、信用支付和还款分别与对应账本模板一致 |
...
| 扫码支付成功率 | `SUCCESS` 的 `QR_PAY`/`CREDIT_PAY` / 已受理扫码交易数 | 扫码、交易、信用和账本事件 | 按资金来源展示且商户入账不重复 |
...
| AT-13 | 新用户注册 | 自动开户并通过复式分录到账 10000.00 元虚拟金 |
...
| AT-20 | H5 确认扫码支付 | 交易 `SUCCESS`，付款方余额减少、商户余额等额增加，并生成借贷平衡分录 |
...
| AT-32 | `QR_PAY` Confirm 入账阶段注入故障 | 自动重试后成功，或补偿至付款方、商户和账本完全一致 |
| AT-33 | 扫码付款方余额不足 | 确认前拒绝，不创建交易、不冻结资金，商户余额不变 |
| AT-34 | 扫码支付密码连续错误 5 次 | 订单在错误期间保持待确认，第 5 次后支付能力锁定且不生成确认令牌 |
| AT-35 | 扫码支付命中前置人工风控 | 进入 `RISK_REVIEW` 且无资金变化；批准后用户重新输入密码并确认 |
| AT-36 | 付款方或商户账户冻结 | 执行前拒绝，订单进入 `REJECTED`，双方资金不变化 |
| AT-37 | 支付、取消和过期任务并发 | 订单版本 CAS 保证只产生一个合法结果；进入处理中后取消和过期失败 |
| AT-38 | `QR_PAY` Try 阶段商户预占失败 | Cancel 释放付款方冻结，订单最终撤销且账本无残缺分录 |
...
| AT-42 | 扫码选择 Mini 花呗支付 88 元 | `CREDIT_PAY SUCCESS`；虚拟余额不变、已用额度+88、商户余额+88、信用应收+88且借贷平衡 |
| AT-43 | Mini 花呗可用额度不足 | 确认前拒绝，不创建交易，不改变余额、额度、商户入账或账本 |
| AT-44 | 对同一扫码订单并发提交 `QR_PAY` 与 `CREDIT_PAY` 各 50 次 | `source_type + source_order_id` 唯一约束保证仅一种资金来源产生一笔交易 |
| AT-45 | `CREDIT_PAY` 商户入账 Try 失败 | Cancel 释放额度冻结，已用额度/商户余额/信用应收均不变 |
...
| 第 5–6 天 | 四类交易 TCC 骨架、幂等、双式账本、固定请求/订单 CAS、额度冻结、Outbox |
| 第 7–8 天 | AI Talk、MCP 工具、多轮澄清、确认令牌 |
| 第 9–10 天 | 风控、Saga/恢复、人工确认、商户扫码、C2C H5 付款、信用支付与还款主流程 |
...
6. **扫码支付（1 分钟）**：商户 Web 创建动态收款码，手机扫码进入 H5、输入支付密码并确认；Web 实时显示结果，对比付款方和商户虚拟余额并展示借贷平衡分录。
7. **个人收款（2 分钟）**：普通用户展示长期个人码，另一用户扫码填写金额并以余额付款；再创建固定请求并演示两名付款人竞争时仅一笔成功、旧码换码后立即失效。
8. **Mini 花呗（2 分钟）**：同一商户收款码选择 Mini 花呗支付，展示虚拟余额不变、商户入账、额度占用和信用应收分录；随后使用虚拟余额部分还款并展示额度恢复。
...
| C2C 与商户扫码边界混淆 | 普通用户绕过信用支付限制 | 独立对象/API/页面，C2C 强制 `BALANCE` 和 `TRANSFER`，服务端拒绝 `MINI_CREDIT` |
| 把会计借贷误解为信用借款 | 账务设计错误 | 明确科目性质和三类记账模板；额度、应收、账单与余额分别建模和对账 |
| 信用支付增加商户余额导致虚拟资金看似增加 | 资金守恒误报 | 同时确认等额信用应收资产；按资产=负债+权益及子账恒等式对账 |
...
8. Web 动态收款码、手机 H5 支付和跨端状态同步可稳定演示；付款方与商户虚拟余额正确变化，`QR_PAY` 账本借贷平衡并可故障恢复。
9. 独立监控章节中的实时、离线、告警和数据质量 P0 能力可用；资金一致性告警、T+1 发布门禁和事件去重均有自动化证据。
10. Mini 花呗固定额度、扫码信用支付、月度账单、部分/全额还款和逾期停用可稳定演示；额度、信用应收、账单、商户余额和复式账本完全一致。
...
| 扫码支付 | 用户扫描商户动态收款码，通过 H5 使用系统虚拟余额向商户虚拟账户付款的 `QR_PAY` 资金交易 |
...
3. 产品不提供用户充值功能，实时指标中的资金初始化单独归类为“虚拟金初始化”，避免与真实充值概念混淆。
