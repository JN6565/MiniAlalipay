...
## 遗留 TODO
1. TCC 内核对接（CreditRepaymentService.submitRepayment）
2. 到期检查分页查询完善（CreditJobService.doDueCheckForAllAccounts）
3. Flyway 迁移在 MySQL 容器中实际执行验证
4. 事件发布（Outbox 模式）：6 种信用事件未实现
5. /actuator/healthcheck 端点应读取 Exchange 中的 requestId（当前缺失）
6. 下游服务统一异常处理：user-center 等 404 仍用 Spring Boot 默认格式，未包装为 ApiResponse
